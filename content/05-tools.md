---
id: tools
title: Tools and Planners
category: tools
summary: Calculators that do the arithmetic for you. Set up your account once and they all work from your own numbers.
tags: tools, calculators, planners
updated: 2026-09-13
---

A wiki tells you that 8 stars costs 475 shards. These work out what *you* need, from where *you* are, and how long it takes at *your* income.

Start with [your account](#/my-account). It takes a minute, it is saved on your device only, and after that every tool and several guides answer in terms of your own Sanctuary, Power and troop tier.

## The planners

| Tool | What it answers |
|---|---|
| [Squad builder](#/squad-builder) | Which five heroes, with live faction bonus, role balance and the bench skill |
| [Hero investment planner](#/hero-planner) | Shards, Antitoxin and weeks to get one hero from A to B |
| [Sanctuary planner](#/sanctuary-planner) | The full bill to your target level, and which prerequisite blocks you |
| [Alliance Duel planner](#/duel-planner) | What your banked items are worth on each day of the Duel |
| [Server timeline](#/server-timeline) | What has unlocked, what is next, and when the Era of Revival arrives |

## Quick calculators

### Alliance Excavation speed

<div data-widget="calc-dig"></div>

Treasure digging is confirmed in-game: player videos show a Treasure Digger tab on the Black Ops screen and chat lines when a member unearths treasure. The speed rule behind this calculator, where time shrinks with the square of the helpers, comes from datamined numbers only, so treat the result as an estimate. Details on the [Alliance Boss and Alliance Excavation](#/alliance-boss-excavation) page.

### Troop promotion

<div data-widget="calc-promotion"></div>

### Shield value

<div data-widget="calc-shield"></div>

### Cheese Trap lure buff

<div data-widget="calc-lure"></div>

## Where the numbers come from

Everything draws on one shared data file built from the public wikis. Shard costs, troop tiers, Duel point values, shield prices, Power gates and the Sanctuary anchors are published figures. Antitoxin per level and the Sanctuary rows between the published anchors are interpolated, and the tools label those as estimates rather than pretending to be exact.

> **KMA:** Found a number that does not match your game? Tell an officer. One correction fixes it everywhere at once, because every guide, chart and calculator reads the same file.
